...
class TestStockGate(unittest.TestCase):
    ...
    def test_lenient_default_sells_on_stale_in_stock(self):
        # Default policy (no live feed): staleness carries no information, so a
        # stale-but-in-stock reading still sells. This is what makes the store
        # usable without an availability engine running.
        g = self.e.gate_order([("A", 1)])
        self.assertTrue(g.accepted)

    def test_lenient_default_still_blocks_known_out_of_stock(self):
        # Even in lenient mode, an item the snapshot says is gone must not sell.
        conn = dbmod.connect(self.path)
        conn.execute("INSERT INTO products (product_id,name,in_stock,price_inr,scraped_at) "
                     "VALUES ('C','Item C',0,100,0)")
        conn.commit(); conn.close()
        g = self.e.gate_order([("C", 1)])
        self.assertFalse(g.accepted)
        self.assertEqual(g.blocked[0]["reason"], "out_of_stock")

    def test_strict_mode_fails_closed_on_stale_data(self):
        # Production behaviour with a live checker: a stale reading is
        # disqualifying and the gate holds the sale.
        strict = StockEngine(db_path=self.path, require_live_confirmation=True)
        g = strict.gate_order([("A", 1)])
        self.assertFalse(g.accepted)
