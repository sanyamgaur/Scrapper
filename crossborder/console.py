...
def _free_batch_id(conn) -> str:
    """A batch id for today that isn't taken yet.

    build_batch() refuses a duplicate id, which is why picking a date by hand
    kept throwing "already exists". Here we just find the next free slot for
    today, so the operator never thinks about dates and can build again the
    moment new orders arrive.
    """
    from datetime import date
    base = date.today().isoformat()
    bid, n = f"PB-{base}", 1
    while conn.execute("SELECT 1 FROM procurement_batches WHERE batch_id=?",
                       (bid,)).fetchone():
        n += 1
        bid = f"PB-{base}-{n}"
    return bid[3:]   # build_batch re-adds the "PB-" prefix


@app.post("/api/ops/fulfil/build-pending")
def fulfil_build_pending() -> dict:
    """Build a batch from every un-batched order — no date to pick.

    This is what makes the buy sheet 'update' after a storefront order: place
    an order, click this, and the new order is sealed into a fresh batch you
    can open. Returns a clear message when there is nothing waiting.
    """
    conn = connect()
    n = conn.execute(
        "SELECT COUNT(*) FROM orders WHERE batch_id IS NULL "
        "AND status IN ('STOCK_CONFIRMED','PLACED')").fetchone()[0]
    if not n:
        conn.close()
        return {"ok": False, "n_orders": 0,
                "error": "No orders waiting to be bought. Place an order on the "
                         "storefront (or seed sample orders) first, then click this."}
    batch_date = _free_batch_id(conn)
    conn.close()
    b = procurement.build_batch(batch_date, operator="console")
    b["ok"] = True
    return b


@app.post("/api/ops/fulfil/oneclick")
def fulfil_oneclick() -> dict:
    """The whole fulfilment funnel in one call, for a fast walk-through:
    build a batch from pending orders -> mark it bought (one line short on
    purpose) -> reconcile. Leaves everything in the DB to inspect after."""
    b = fulfil_build_pending()
    if not b.get("ok"):
        return b
    bid = b["batch_id"]
    sim = demo_simulate_buys(SimulateReq(batch_id=bid, short_one=True))
    rec = procurement.reconcile(bid)
    return {"ok": True, "batch_id": bid,
            "orders": b.get("n_orders"), "lines": b.get("n_lines"),
            "carts": b.get("n_carts"),
            "bought_lines": sim.get("bought"), "short_lines": sim.get("short"),
            "orders_complete": rec.get("orders_complete"),
            "orders_short": rec.get("orders_short"),
            "note": "Built → bought → reconciled. Open the buy sheet or pack-out "
                    "to see the detail."}


@app.get("/api/ops/batch/{batch_id}/cart-summary")
def cart_summary(batch_id: str) -> dict:
    """One consolidated shopping list per dark-store cart.

    Blinkit has no public 'prefill my cart' URL — a cart is bound to a phone,
    a login and a saved address the project doesn't hold (see operator.py). So
    the honest 'one link' is ONE operator page per cart that lists everything
    with quantities and a running total, plus a copy-ready shopping list the
    buyer can work down in the Blinkit app. That beats N separate product tabs.
    """
    from .operator import OperatorFlow
    ps = OperatorFlow().pick_sheet(batch_id)
    carts = []
    for c in ps.get("carts", []):
        lines = c["lines"]
        units = sum(l["qty"] for l in lines)
        txt = "\n".join(
            f"{l['qty']} x  {l['name']}   (~Rs {round(l['expected_inr'])})"
            for l in lines)
        carts.append({
            "cart_no": c["cart_no"], "merchant_id": c["merchant_id"],
            "n_items": len(lines), "n_units": units,
            "value_inr": round(c["value_inr"], 2),
            "shopping_list": txt,
            "operator_url": f"/operator?batch={batch_id}",
            "blinkit_url": "https://blinkit.com/",
            "lines": lines,
        })
    return {"batch_id": batch_id, "state": ps.get("state"),
            "done": ps.get("done"), "total": ps.get("total"), "carts": carts}
