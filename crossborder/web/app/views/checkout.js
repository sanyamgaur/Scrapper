    ...
    try {
      ...
      if (e.status === 409 && gate) {
        ...
        const advice = blockedAll
          ? "You haven't been charged. Please try again shortly."
          : "Remove the item(s) above and your other items will check out normally.";
        const head = blockedAll
          ? "A couple of items just sold out in Delhi"
          : "One item just sold out in Delhi";
        err.innerHTML = `<div class="note bad" style="margin-top:14px">
          <b>${esc(head)}.</b> We only charge for what we can actually buy for you.${msgs}
